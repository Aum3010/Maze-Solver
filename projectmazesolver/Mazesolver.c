...
void image_writer_in()
{
    ...
    FILE* pgmimg_in;
    pgmimg_in = fopen("maze_pgmimg_in.pgm", "wb");
  
    // Writing Magic Number to the File
    fprintf(pgmimg_in, "P2\n"); 
  
    // Writing Width and Height
    fprintf(pgmimg_in, "%d %d\n", width, height); 
  
    // Writing the maximum gray value
    fprintf(pgmimg_in, "255\n"); 
    int count = 0;
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
...
void image_writer_out()
{
    ...
    FILE* pgmimg_out;
    pgmimg_out = fopen("maze_pgmimg_out.pgm", "wb");
  
    // Writing Magic Number to the File
    fprintf(pgmimg_out, "P2\n"); 
  
    // Writing Width and Height
    fprintf(pgmimg_out, "%d %d\n", width, height); 
  
    // Writing the maximum gray value
    fprintf(pgmimg_out, "255\n"); 
    int count = 0;
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
            temp = arr_out_t[i][j];
